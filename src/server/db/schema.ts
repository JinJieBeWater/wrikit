...
export const pageEnum = pgEnum("pageTy", ["md", "pure", "item", "iframe"]);
...
export const pagesToChildren = createTable(
  ...
  (example) => ({
    pageIdIdx: index("page_id_id_idx").on(example.pageId),
    childIdIdx: index("child_id_idx").on(example.childId),
  }),
);
